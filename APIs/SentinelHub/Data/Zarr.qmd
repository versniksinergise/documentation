...
geometries](/api/byoc/#a-note-about-cover-geometries) (other than the
envelope), thus all data arrays are considered to cover the full Zarr
envelope. Thus, the value of `dataMask` is always 1 inside the Zarr\'s
envelope and 0 outside. Areas for which an array has no data chunks are
filled with the no data value.
